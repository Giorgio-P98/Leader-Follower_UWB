...
void MocapPX4Bridge::poseCallback(const geometry_msgs::msg::PoseStamped::UniquePtr poseMsg){
	...
	*/
	vehicle_odometry_Msg.position[0] = (poseMsg->pose.position.x - x_ground);
	vehicle_odometry_Msg.position[1] = (poseMsg->pose.position.y - y_ground);
	vehicle_odometry_Msg.position[2] = (poseMsg->pose.position.z - z_ground);

	vehicle_odometry_Msg.q[0] = poseMsg->pose.orientation.w;
	vehicle_odometry_Msg.q[1] = poseMsg->pose.orientation.x;
	vehicle_odometry_Msg.q[2] = poseMsg->pose.orientation.y;
	vehicle_odometry_Msg.q[3] = poseMsg->pose.orientation.z;
